...
//   	expect("Something dude was").toEqual(upper.children().last().text());
  ...
  it("15.1 should be able to redistribute only periods ", function() {
 	   var html = $('<div class="book"><div id="upper-container" class="upper"></div>'+
	   '<div id="text-bucket-styled" class="reader"></div>'+
	   '<div id="text-bucket-end" class="downer">'+
	   '<p>You provoked me beyond all reasonableness, even for a barbarian. You shout like a lunatic, upset my mother, interrupt my house’s tranquillity, disturb the servants, and my wife’s already had to replace one shoji door. I could not possibly permit your obvious lack of manners to go unopposed. Or allow you to go against my wishes in my own house. \n</p></div>'
	   +'</div>')
	  var bottom = $("#text-bucket-end",html);
	  var readerr = $("#text-bucket-styled",html);
	  var upper = $("#upper-container",html);

	  reader.redistributing_up(bottom,readerr,6)
      expect("You provoked me beyond all reasonableness,").toEqual(readerr.children().first().text());
      expect(bottom.children().first().text().includes("even for a barbarian. You shout "));

      reader.redistributing_up(readerr,upper,6)
      expect("").toEqual(readerr.text());

      reader.redistributing_up(bottom,readerr,6)
        expect(" even for a barbarian. ").toEqual(readerr.children().first().text());
...
it("16.1 should find first index of a mark (like period,carriage,parenthesis,comma)", function() {
  ...
  it("16.1.1 should   find mark but realize is a period", function() {
    ...
    it("16.1.2 should find first index of a mark (like period,carriage,parenthesis,comma)", function() {
      var text = " You provoked me beyond all reasonableness, even for a barbarian. You shout like a lunatic"
      		expect(65).toEqual(reader.find_mark(text,["\n","."]))
